...
%-----SA-----%
function [X_best, f_best, f_individual_best, Y] = SA(dj, Dk, m, J, K, max_iter, dij, djj, tempo_recarga, velocidade, Tk, Vk, Vp, dcc, ppc, casas, indice_casas, indice)
  ...
  X_best = Xijk;
  inicial = 1
  [f_best, f_individual_best] = calcularObjetivo(Xijk, dj, K, dij, djj, Dk_bateria, Dk, tempo_recarga, velocidade, Tk, Tk_tanque, Vk, Vp, dcc, ppc, casas, indice_casas)
  f_curr = 0;
  ...
  while T > Tf && iter <= max_iter
    ...
    X_ijk = buscaLocal(X_ijk, K);
     ...
     [f_curr, f_individual_curr] = calcularObjetivo(X_ijk, dj, K, dij, djj, Dk_bateria, Dk, tempo_recarga, velocidade, Tk, Tk_tanque, Vk, Vp, dcc, ppc, casas, indice_casas);
     delta_f = f_curr - f_best;

     if delta_f < 0 && ~verificarRestricoes(X_ijk, J, indice)
       iter
        X_best = X_ijk;
        f_best = f_curr
     elseif exp(-delta_f / T) > rand() && ~verificarRestricoes(X_ijk, J, indice)
        X_aceito = X_ijk;
        f_aceito = f_curr;
        aceito = true;
     endif
...
tic;
[X_best, f_best, f_individual_best, Y] = SA(dj, Dk, m, J, K, max_iter, dij, djj, tempo_recarga, velocidade, Tk, Vk, Vp, dcc, ppc, casas, indice_casas, indice);
tempo_CPU = toc
