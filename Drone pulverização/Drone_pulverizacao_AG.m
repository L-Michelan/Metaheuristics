...
% crossover e mutacao
  ...
  % alicar mutacao se chance for cumprida
    if probabilidade_mutacao > rand
      ...
      % avalia novos individuos
      if verificarRestricoes(filho_mutado1, J, indice)
        f_mutado1 = Inf; % penaliza solucoes que violam as restricoes
      elseif restricao_sobrecarga(filho_mutado1, J, indice)
        [f_mutado1, ~] = calcularObjetivo(filho_mutado1, dj, K, dij, djj, Dk_bateria, Dk, tempo_recarga, velocidade, Tk, Tk_tanque, Vk, Vp, dcc, ppc, casas, indice_casas);
        f_mutado1 += 600;
      else
        [f_mutado1, ~] = calcularObjetivo(filho_mutado1, dj, K, dij, djj, Dk_bateria, Dk, tempo_recarga, velocidade, Tk, Tk_tanque, Vk, Vp, dcc, ppc, casas, indice_casas);
      endif

      if verificarRestricoes(filho_mutado2, J, indice)
        f_mutado2 = Inf; % penaliza solucoes que violam as restricoes
      elseif restricao_sobrecarga(filho_mutado2, J, indice)
        [f_mutado2, ~] = calcularObjetivo(filho_mutado2, dj, K, dij, djj, Dk_bateria, Dk, tempo_recarga, velocidade, Tk, Tk_tanque, Vk, Vp, dcc, ppc, casas, indice_casas);
        f_mutado2 += 600;
      else
        [f_mutado2, ~] = calcularObjetivo(filho_mutado2, dj, K, dij, djj, Dk_bateria, Dk, tempo_recarga, velocidade, Tk, Tk_tanque, Vk, Vp, dcc, ppc, casas, indice_casas);
      endif
