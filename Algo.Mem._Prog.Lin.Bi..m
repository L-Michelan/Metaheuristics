...
p=6 %input ('Tamanho da populaçao ')%
a=0; %matriz populacao%
c=0;%definindo variavel da funcao a ser minimizada%
R=0; %vetor restriçao 0=nao 1=sim
s=1+round(rand*p) %tamanho populacao para cruzamento%
b=0; %matriz populacao cruzamento%
P=0; %populacao aumentada
H=[0,0,0,0;0,0,0,0];
E=0;
r=rand;
F=p+4;
...
for i=1:p
  for j=1:n
    a(i,j)=0;
    B(j)=0;
    Q(j)=0 %Vetor da seleçao cultural%
    for I=1:p+1
    E(I,j)=0;
    endfor
    e(i)=0;
...
if f(i)>e(o)
  ...
  e(o)=f(i);
  E(o,j)=B(i,j);
  if f(i)<e(i)+0.1*e(i) && f(i)>=e(i)-0.1*e(i)
    Q(j)=B(i,j); %cultural%
  endif
endfor
endif
...
if (r<0.05) #pop aumentada se mutou
  for i=p+2:F
    for l=1:2
      for j=1:n
      P(i,j)=H(l,j);
      P(F+1,j)=Q(j);#Adiciona indiv selecionado culturalmente na pop aumentada p/ possiveis futuros cruzamentos
    endfor
  endfor
endfor
%---------------------------Fim----------------------------%
