...
function [best_solution, best_cost, initial_solution, best_eq_values] = sa_sistema_nao_linear()
    ...
    for iter = 1:max_iter
        % Gera uma solução vizinha aleatória dentro dos limites
        neighbor_solution = generate_neighbor(current_solution, lower_bound, upper_bound);

        % Calcula o custo da solução vizinha
        [neighbor_cost, neighbor_eq_values] = cost_function(neighbor_solution);

        % Calcula a variação no custo
        delta_cost = neighbor_cost - best_cost;

        % Aceita a solução vizinha se ela for melhor ou de acordo com a probabilidade de Boltzmann
        if delta_cost < 0 || rand() < exp(-delta_cost / initial_temperature)
...
function neighbor = generate_neighbor(current_solution, lower_bound, upper_bound)
    % Gera uma solução vizinha aleatória dentro dos limites
...
[best_solution, best_cost, initial_solution, best_eq_values] = simulated_annealing();
