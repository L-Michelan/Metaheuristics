...
function [best_solution, best_cost, best_eq_values, first_best_solution] = ma_sistema_nao_linear()

    % Define os parâmetros do algoritmo memético
    ...
    hill_climbing_rate = 0.1; % Taxa de aplicação da busca local (hill climbing)
    ...
    % Inicia o loop principal do algoritmo memético
    for generation = 1:max_generations
        ...
        % Aplica busca local (hill climbing) em uma fração dos filhos mutados
        num_hill_climbing = ceil(hill_climbing_rate * size(mutated_children, 1));
        for i = 1:num_hill_climbing
            mutated_children(i, :) = hill_climbing(mutated_children(i, :), lower_bound, upper_bound);
        end
...
function [new_solution] = hill_climbing(solution, lower_bound, upper_bound)
    % Aplica a busca local hill climbing
    num_variables = numel(solution);
    new_solution = solution;
    for i = 1:num_variables
        if rand() < 0.5 % Escolhe aleatoriamente uma direção para explorar
            step = 0.1 * (upper_bound(i) - lower_bound(i));
        else
            step = -0.1 * (upper_bound(i) - lower_bound(i));
        end
        new_solution(i) = max(lower_bound(i), min(upper_bound(i), new_solution(i) + step));
    end
end
...
[best_solution, best_cost, best_eq_values, first_best_solution] = ma_sistema_nao_linear();
